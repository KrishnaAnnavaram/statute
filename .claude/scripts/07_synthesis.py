...
RULE_ORIGIN_LABELS = {
    "conditional_branch": "IF/ELSIF branch",
    "case_branch": "CASE expression branch",
    "named_exception": "guarded RAISE",
    "predefined_exception": "predefined exception handler",
    "failure_isolation": "WHEN OTHERS (per-record isolation)",
    "error_contract": "WHEN OTHERS (application error)",
    "generic_exception": "WHEN OTHERS handler",
    "cursor_eligibility": "cursor WHERE clause",
    "variable_derivation": "assignment / backward slice",
    "ddl_check_constraint": "CHECK constraint",
    "ddl_virtual_column": "virtual column expression",
    "ddl_unique_constraint": "UNIQUE constraint",
    "ddl_unique_index": "UNIQUE index",
    "ddl_view_filter": "view WHERE clause",
}


def rule_source_phrase(src: dict, object_names: dict, source_files: dict = None) -> str:
    """
    Where a rule came from, precise enough for a technical reader to open it.

    A rule that cites only "Transfer Funds" cannot be checked; one that cites
    `05_medium_fund_transfer_with_validation.sql:79` can be, in seconds. The
    construct is named too, so a reviewer knows whether they are looking at a
    CHECK constraint or a guarded RAISE before they open anything.
    """
    kind = src.get("kind", "")
    construct = RULE_ORIGIN_LABELS.get(kind, kind.replace("_", " "))

    if kind.startswith("ddl_"):
        target = src.get("table") or src.get("view") or "schema"
        detail = src.get("constraint_name") or src.get("index_name") or src.get("column")
        named = f" `{detail}`" if detail else ""
        return f"{construct}{named} on `{target.upper()}` (DDL)"

    oid = src.get("object_id", "")
    unit = object_names.get(oid, object_title(oid))
    line = src.get("line")
    fname = (source_files or {}).get(oid)
    where = f"`{fname}:{line}`" if fname and line else (f"line {line}" if line else "")
    return f"{construct} in {unit}" + (f" — {where}" if where else "")
...
def build_document(ctx: dict) -> str:
    ...
    d.table(["Item", "Value"], [
        ...
        ["Generated by", "Deterministic static-analysis pipeline. Python analysers "
                         "(ANTLR4 Oracle PL/SQL grammar for structure, sqlglot for SQL) "
                         "orchestrated by an AI agent harness."],
        ["Hallucination risk", "**None — structurally impossible.** No language model "
                               "generates, summarises, judges or rewrites any content here. "
                               "Every sentence is assembled by rule from a parse tree, which "
                               "is why every statement cites a file and line number. There is "
                               "no sampling temperature because there is no model in the "
                               "generation path."],
    ...
    d.p("**Status note.** This document records what the code does, not what the business "
        "intends. Section 13 lists everything that still requires input from a person.")
    ...
    d.p(f"**{ctx['system_name']}** is a banking system that performs "
        ...
        f"**{len(tables)} database tables**.")
    d.p()
    d.p("The system provides:")
    for c in caps:
    ...
    d.table(["Included", "Count", "Meaning"], [
        ["Program units", len(caps), "Procedures and functions analysed"],
        ["Business rules", len(rules), "Decisions, calculations, limits and checks it enforces"],
        ["Tables", len(tables), "Database tables the system reads or writes"],
        ["Columns", ctx["stats"]["columns"], "Columns across all tables"],
        ["Parameters", ctx["stats"]["parameters"], "IN / OUT parameters across all units"],
    ])
    ...
    d.h(3, "4. Program Units")
    d.p("Each procedure and function, with what it does and what it accesses. Full specifications are in Part III.")
    for c in caps:
    ...
    if dt:
        d.h(4, "Figure 1 — Data flow: program units and the tables they access")
        d.mermaid(dt)
    ...
    for cat in ["VALIDATION", "LIMIT_CHECK", "CALCULATION", "ROUTING", "COMPLIANCE", "ERROR_HANDLING"]:
        ...
        for r in sorted(group, key=lambda x: x["rule_id"]):
            ...
            d.table(["Attribute", "Value"], [
                ...
                ["Source", rule_source_phrase(r["source"], object_names, ctx["source_files"])],
    ...
    d.table(["Reference", "Guarantee", "Where"],
            [[r["rule_id"], sentence_case(humanise_description(r["description"], object_names, ctx["table_titles"]).split(".")[0]),
              rule_source_phrase(r["source"], object_names, ctx["source_files"])] for r in err_rules])
    ...
    if erd and erd.exists():
        d.h(4, "Figure 3 — Entity Relationship Diagram (ERD)")
    ...
    for tname in sorted(tables):
        ...
        d.table(["Column", "Name in DDL", "Oracle type", "Target type", "Constraints", "Default"], rows)
    ...
    if seqs:
        d.h(4, "8.x Sequences")
        d.p("Oracle sequences. A rebuild must preserve the current high-water marks to avoid key collisions.")
        d.table(["Sequence", "START WITH", "INCREMENT BY", "CACHE", "Used by column"],
    ...
    d.p("This document was generated by a deterministic static-analysis pipeline: a set of "
        "Python analysers orchestrated by an AI agent harness. The harness runs the stages "
        "and reviews the code; **no language model generates, summarises, judges or rewrites "
        "any content in this document**.")
    d.p()
    d.p("Structure is parsed with the ANTLR4 grammar for Oracle PL/SQL; individual SQL "
        "statements are broken down with sqlglot in Oracle dialect. Every sentence below is "
        "assembled by rule from the resulting parse trees. **There is no sampling temperature "
        "to tune, because there is no model in the generation path** — the same source always "
        "produces the same document, and hallucination is not reduced but structurally "
        "impossible. That is what makes it safe for every statement to cite a file and line "
        "number a reviewer can open.")
    d.p()
...
def build_context(args, artifacts) -> dict:
    ...
    # Rule provenance is only checkable if it names the file, not just the unit.
    file_names = {fid: Path(meta.get("rel_path") or meta.get("abs_path", "")).name
                  for fid, meta in (inventory.get("file_metadata") or {}).items()}
    source_files = {oid: file_names.get(rec.get("file_id"), "")
                    for oid, rec in parser_records.items()}
